...
const navigate = useNavigate()
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-grey-900 md:mx-10'>
        <h1 className='text-3xl text-green-600 font-medium'>Top {speciality} For You</h1>
        ...
        <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
            {doctors.map((item,index)=>(
               item.speciality == speciality ? <div onClick={()=>navigate(`/appointment/doc${item.id}`)} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500'>
               <img className='bg-blue-50' src={DocImages[item.image]} alt="" />
               <div className='p-4'>
        ...
        <button onClick={()=>{ navigate(`/doctors`); scrollTo(0,0)}} className='bg-green-300 text-grey-600 px-12 py-3 rounded-full mt-10'>All Doctors</button>
    </div>
  )
}
